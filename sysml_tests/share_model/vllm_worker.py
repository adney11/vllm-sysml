...
def process_requests(engine: LLMEngine,
                     test_prompts: List[Tuple[str, SamplingParams]], rank):
...
def initialize_engine(args: argparse.Namespace, shared_model=None) -> LLMEngine:
    """Initialize the LLMEngine from the command line arguments."""
    engine_args = EngineArgs.from_cli_args(args)
    return LLMEngine.from_engine_args(engine_args, shared_model=shared_model)
...
def worker_main(args: argparse.Namespace, **kwargs):
    ...
    args.gpu_memory_utilization = 0.225
    # args.gpu_memory_utilization = 0.9
    # args.tensor_parallel_size = 4
    engine = initialize_engine(args, kwargs["shared_model"])
    test_prompts = create_test_prompts()
    start = time.time()
    process_requests(engine, test_prompts, rank)
    stop = time.time()
    print(f"[Rank {rank}]: Completed {BATCH_SIZE} inf requests in {stop - start} sec.")
