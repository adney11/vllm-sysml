...
PROFILE_SECTION = "not_nsys" # "promspt", "token", "inf", "not_nsys"
...
def create_test_prompts() -> List[Tuple[str, SamplingParams]]:
    ...
    with open(os.path.join(tests_path, "random_prompts_1000_1024tok_opt125m.csv"), "r") as fr:
...
def process_requests(engine: LLMEngine,
    ...
    for test_prompt in test_prompts:
        prompt, sampling_params = test_prompt
    ...
    #             request_id += 1
        ...
        if step == 0: # step BATCH_SIZE-1 = last prompt processing step:
...
def main(args: argparse.Namespace):
    ...
    args.gpu_memory_utilization = 0.1125
    engine = initialize_engine(args)
    test_prompts = create_test_prompts()
    start = time.time()
    process_requests(engine, test_prompts)
    stop = time.time()
    print(f"Completed {BATCH_SIZE} inf requests in {stop - start} sec.")
...
if __name__ == '__main__':
    ...
    main(args)
    stop= time.time()
    print(f"Total elapsed time: {stop - start} sec")
